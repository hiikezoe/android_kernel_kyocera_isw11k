...
#define KERR_SMEM_LOG_SIZE        (64*1024)
...
static int kerrsmem_read(struct file *file, char __user *buf, size_t count,
              loff_t *ppos)
{
    int ret = 0;
    int retry_cnt = 0;
...
static int kerrsmem_write(struct file *file, const char __user *buf,
                  size_t count, loff_t *ppos)
{
    printk(KERN_INFO "kerrsmem_write\n");
    printk(KERN_INFO "Unsoported operation!\n");

    return 0;
}

static int kerrsmem_ioctl(struct inode *inode, struct file *filp,
...
static int __init kerrsmem_init(void)
{
    ...
    if (kerr_devp) {
        ...
        strlcpy(kerr_devp->name, "kclog_smem", 10);
