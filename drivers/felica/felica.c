...
static int pon_open(struct inode *inode, struct file *file)
{
	...
	memset( &termios, 0x00, sizeof( struct ktermios ));

	if( 0 != g_unrate ){
		termios.c_cflag |= g_unrate;
		...
		termios.c_cflag |= g_unbaud;

		termios.c_ispeed = g_unrate;
		termios.c_ospeed = g_unrate;

		unRet = uart_set_ttyhs1_termios(&termios);
		
	}

	nRet = 0;
	MFC_DRV_DBG_LOG("uart_set_ttyhs1_baud_rate Ret = %d \n", unRet );
...
static __init int felica_init(void)
{
	...
	i2c_info.addr = I2C_MFD_SLAVE_ADDR;

	strlcpy(i2c_info.type, SH_MFD_CEN ,sizeof(i2c_info.type) );

	padapter = i2c_get_adapter(I2C_BUS_NUMBER);
	if (!padapter) {
		MFC_DRV_ERR_LOG("Felica can't get i2c adapter \n");
		return -1;
	}
	this_client = i2c_new_device(padapter, &i2c_info);
	if(!this_client){
		MFC_DRV_ERR_LOG("Felica can't new i2c device \n");
		return -1;
	}
