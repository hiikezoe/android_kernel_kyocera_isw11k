...
static uint32 vee_seq_record_len = sizeof(uint32)*2;
static uint32 mddi_quickvx_vee_on_seq_tbl[MDDI_QUICKVX_VEE_SEQ_MAX][2] = {
    { 0x00020000, 0x000001EF },
    { 0x00030020, 0x00001FF8 },
    { 0x00000000, 0x00000001 },
    { 0x0000000C, 0x00000000 },
};

static uint32 mddi_quickvx_vee_off_seq_tbl[MDDI_QUICKVX_VEE_SEQ_MAX][2] = {
    { 0x0000000C, 0x00000000 },
    { 0x00000000, 0x00000000 },
    { 0x00030020, 0x00001FF9 },
    { 0x00020000, 0x000001EC },
};
...
{
    0x000000FA, 0x00000102, 0x00000118, 0x00000108, 0x00000124, 0x0000014A,
    0x00000138, 0x00000129, 0x000001C2, 0x000001C7, 0x000001BA, 0x000001C1,
    0x000001C7, 0x000001B8, 0x000001CC, 0x000001D5, 0x000001CB, 0x00000100,
    0x0000014B, 0x00000100, 0x00000144, 0x00000100, 0x0000015D
};
...
{
    0x000000FA, 0x00000102, 0x00000118, 0x00000108, 0x00000124, 0x0000014A,
    0x00000138, 0x00000129, 0x000001C0, 0x000001C5, 0x000001B6, 0x000001BD,
    0x000001C5, 0x000001B4, 0x000001CA, 0x000001D3, 0x000001C9, 0x00000100,
    0x00000157, 0x00000100, 0x00000150, 0x00000100, 0x0000016B
};
...
{
    0x000000FA, 0x00000102, 0x00000118, 0x00000108, 0x00000124, 0x0000014A,
    0x00000138, 0x00000129, 0x000001BF, 0x000001C5, 0x000001B4, 0x000001BB,
    0x000001C4, 0x000001B2, 0x000001C9, 0x000001D2, 0x000001C8, 0x00000100,
    0x0000015D, 0x00000100, 0x00000156, 0x00000100, 0x00000172
};
...
{
    0x000000FA, 0x00000102, 0x00000118, 0x00000108, 0x00000124, 0x0000014A,
    0x00000138, 0x00000129, 0x000001BF, 0x000001C4, 0x000001B3, 0x000001BA,
    0x000001C3, 0x000001B1, 0x000001C8, 0x000001D1, 0x000001C7, 0x00000100,
    0x00000163, 0x00000100, 0x0000015B, 0x00000100, 0x00000179
};
...
{
    0x000000FA, 0x00000102, 0x00000118, 0x00000108, 0x00000124, 0x0000014A,
    0x00000138, 0x00000129, 0x000001BE, 0x000001C4, 0x000001B2, 0x000001B9,
    0x000001C2, 0x000001B0, 0x000001C7, 0x000001D0, 0x000001C6, 0x00000100,
    0x00000169, 0x00000100, 0x00000160, 0x00000100, 0x00000180
};
...
static void mddi_quickvx_vee_other_degine_seq(
    mddi_quickvx_ctrl_e_type ctrl,
    uint32 strength
);
...
static void (*vee_seq_func)(mddi_quickvx_ctrl_e_type ctrl,uint32 strength)
    = mddi_quickvx_vee_other_degine_seq;
...
static void mddi_quickvx_get_revision(void)
{
    ...
    DISP_LOCAL_LOG_EMERG("DISP: Get rev wr:reg[%X]\n",(int)(reg_addr));

    ret = i2c_master_recv( i2c_quick_client,
                           (char*)(&rd_value),
                           sizeof(rd_value) );

    DISP_LOCAL_LOG_EMERG("DISP: Get rev rd:data[%X]\n",(int)(rd_value));

    if( ret > 0 )
    {
        mddi_quickvx_ic_rev = rd_value;

        if( rd_value >= QUICKVX_NEW_DEGINE_REV )
        {
            vee_seq_func = mddi_quickvx_vee_new_degine_seq;
        }
        else
        {
            vee_seq_func = mddi_quickvx_vee_other_degine_seq;
        }
    }
...
===========================================================================*/
static void mddi_quickvx_vee_wait_block_switch( void )
{
    int i;
    int ret;
    ...
    for(i=0; i<QUICKVX_VEE_SWITCHING_RETRY; i++)
    {
       ...
       DISP_LOCAL_LOG_EMERG("DISP: wait vee switch wr:reg[%X]\n",(int)(reg_addr));

       ret = i2c_master_recv( i2c_quick_client,
                              (char*)(&rd_value),
                              sizeof(rd_value) );

       DISP_LOCAL_LOG_EMERG("DISP: wait vee switcht rev rd:data[%X]\n",(int)(rd_value));
...
)
{
    ...
    if( ON == ctrl )
    {
        ...
        i2c_master_send( i2c_quick_client,
                         (char*)buff,
                         sizeof(buff) );
        DISP_LOCAL_LOG_EMERG("DISP: I2C wr: reg[%X] data[%X]\n",
                (int)(buff[0]),(int)(buff[1]));

        /* wait */
        mddi_quickvx_vee_wait_block_switch();

        /* VEE Control Register 0 */
        buff[0] = QUICKVX_VEE_VEECTRL_REG;
        buff[1] = QUICKVX_VEE_CTRL_ON;
        ...
        buff[0] = QUICKVX_VEE_STRENGTH_REG;
        buff[1] = strength;
...
FUNCTION  MDDI_QUICKVX_VEE_OTHER_DEGINE_SEQ
...
)
{
    ...
    if( ON == ctrl )
    {
        mddi_quickvx_vee_on_seq_tbl[MDDI_QUICKVX_VEE_SEQ_OFFSET][1] = strength;

        DISP_LOCAL_LOG_EMERG("DISP: vee on (other degine)\n");
        DISP_LOCAL_LOG_EMERG("DISP: vee strength:[%d]\n",
               (int)((mddi_quickvx_vee_on_seq_tbl[MDDI_QUICKVX_VEE_SEQ_OFFSET][1])>>3));
        for(i=0; i<MDDI_QUICKVX_VEE_SEQ_MAX; i++)
        {
            i2c_master_send( i2c_quick_client,
                             (char*)mddi_quickvx_vee_on_seq_tbl[i],
                             vee_seq_record_len );
            DISP_LOCAL_LOG_EMERG("DISP: vee seq:[%x][%x]\n",
                   (int)mddi_quickvx_vee_on_seq_tbl[i][0],
                   (int)mddi_quickvx_vee_on_seq_tbl[i][1] );
        }
    }
    else
    {
        DISP_LOCAL_LOG_EMERG("DISP: vee off (other degine)\n");
        for(i=0; i<MDDI_QUICKVX_VEE_SEQ_MAX; i++)
        {
            i2c_master_send( i2c_quick_client,
                             (char*)mddi_quickvx_vee_off_seq_tbl[i],
                             vee_seq_record_len );
            DISP_LOCAL_LOG_EMERG("DISP: vee seq:[%x][%x]\n",
                   (int)mddi_quickvx_vee_off_seq_tbl[i][0],
                   (int)mddi_quickvx_vee_off_seq_tbl[i][1] );
        }
    }
...
)
{
    ...
    {
        if(NULL != vee_seq_func)
        {
            (*vee_seq_func)( ctrl, strength );
        }
...
void mddi_quickvx_configure_registers(void)
{
    ...
    for(loop = 0; loop < MDDI_LOCAL_CRC_ERR_CHECK_RETRY; loop++)
    {
        ...
        ql_mddi_write(QUICKVX_VEE_STRENGTH_REG, 0x00000000);
        /* Video Enhancement Enable Register */
